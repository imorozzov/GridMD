 ...
 *
 *   $Revision: 1.11 $
 *   $Date: 2012/09/06 15:00:18 $
 *   @(#) $Header: /home/plasmacvs/source_tree/gridmd/include/gmfork.h,v 1.11 2012/09/06 15:00:18 valuev Exp $
 *
 *****************************************************************************/
/*
$Source: /home/plasmacvs/source_tree/gridmd/include/gmfork.h,v $
$Revision: 1.11 $
$Author: valuev $
$Date: 2012/09/06 15:00:18 $
*/
/*s****************************************************************************
 * $Log: gmfork.h,v $
 * Revision 1.11  2012/09/06 15:00:18  valuev
 * minor changes
 *
 * Revision 1.10  2012/08/31 15:15:44  valuev
 * restructured gridmd workflow
 *
 * Revision 1.9  2012/01/17 06:00:07  valuev
 * neb workflow
 *
 * Revision 1.8  2011/01/25 10:07:35  valuev
 * some new fixes
 *
 * Revision 1.7  2010/11/17 02:17:16  valuev
 * added file links
 *
 * Revision 1.6  2010/10/14 15:02:29  morozov
 ...
 *
 * Revision 1.3  2010/09/17 19:36:18  valuev
 * changed fork function names, added regular expressions
 *
 * Revision 1.2  2010/09/17 12:24:43  valuev
 * updated fork
 *
...
public:
  ...
  ///     All existing fork branches and their data are deleted by calling this function.
  ...
  int begin(const string &name_, gmSelector input=gmNODE_PREV, const link_type &t=gmHardLink(), int srcport=-1){
    ...
    if(splname=="") // automatic name assignement
      splname=fmt("%sbegin",name=="" ? "" : (const char *)(name+".").c_str()); // name from the fork name and branch id 
  ...
  int split(const string &name_=""){
    ...
    if(splname=="") // automatic name assignement
      splname=fmt("%ssplit(%d)",name=="" ? "" : (const char *)(name+".").c_str(),bcount); // name from the fork name and branch id 
  ...
  ///     (nonzero for processing request in the worker mode, 0 otherwise) may be analyzed
  ///     to trigger merge node execution.
  ...
  int merge(const string &name_, gmSelector input=gmNODE_PREV, const link_type &t=gmHardLink(), int srcport=-1,bool same_ports=false){
    ...
    if(splname==""){ // automatic name assignement
      ...
      if(!bcount || branches[bcount-1].end>=0) // the branch bcount already begun, but there were no split node
        bnum=bcount;
      splname=fmt("%smerge(%d)",name=="" ? "" : (const char *)(name+".").c_str(),bnum); // name from the fork name and branch id 
    }
    int res;
  ...
  int end(const string &name_="", bool same_ports=false){
    string splname=name_;
    if(splname=="") // automatic name assignement
      splname=fmt("%send",name=="" ? "" : (const char *)(name+".").c_str()); // name from the fork name
    int res=sys->mark_node(splname,gmNODE_NONE);
    end_here(same_ports);
    return res;   
  }
  ...
  split_t &vbegin_out(int branchid) const{
    if(gmlink_traits<split_t>::has_value){
      ...
      if(top_node==sys->get_curnode())
        ...
        LOGMSG(vblWARN,fmt("gmFork<>.vsplit_out: invalid call outside end node of '%s', using uninitialized data!",(const char *)name.c_str()),0);
    }
    else
      LOGMSG(vblWARN,fmt("gmFork<>.vsplit_out: invalid call for hard link in '%s', using uninitialized data!",(const char *)name.c_str()),0);
    return tmp_split;
  ...
  const split_t &vsplit_in() const{
    ...
    if(bcount && branches[bcount-1].beg==sys->get_curnode())
      ...
      LOGMSG(vblWARN,fmt("gmFork<>.vsplit(in): invalid call outside split node of '%s', using uninitialized data!",(const char *)name.c_str()),0);
      return tmp_split;
    }
  }
  ...
  branch_t &vsplit_out(){
    ...
    LOGMSG(vblWARN,fmt("gmFork<>.vbranch_out: invalid call outside split node of '%s', using uninitialized data!",(const char *)name.c_str()),0);
    return tmp_branch;
  }
  ...
  const branch_t &vmerge_in() const {
    ...
    LOGMSG(vblWARN,fmt("gmFork<>.vbranch_in: invalid call outside merge node of '%s', using uninitialized data!",(const char *)name.c_str()),0);
    return tmp_branch;
  }
  ...
  merge_t &vmerge_out(int portid=0){
    ...
    if(bcount && (branches[bcount-1].end==this_node || (branches[bcount-1].beg==this_node && branches[bcount-1].end<0) ) )
      ...
      LOGMSG(vblWARN,fmt("gmFork<>.vmerge(out): invalid call outside merge node of '%s', using uninitialized data!",(const char *)name.c_str()),0);
      return tmp_merge;
    }
  }
  ...
  const merge_t &vend_in(int branchid) const {
    ...
    LOGMSG(vblWARN,fmt("gmFork<>.vmerge(in): invalid call outside end node of '%s', using uninitialized data!",(const char *)name.c_str()),0);
    return tmp_merge;
  }
  ...
  int get_splitnode(int branchid) const {
    if(branchid<0 || branchid>=(int)branches.size()){
      LOGMSG(vblWARN,fmt("gmFork<>.get_splitnode: branch id %d for '%s' is outside the valid range [0,%d]!",branchid,(const char *)name.c_str(),branches.size()-1),0);
      return gmNODE_NONE;
    }
